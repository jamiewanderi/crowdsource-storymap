DROP FUNCTION IF EXISTS usercomments_upsert(int[], text[]);  
...
-- inserted: 1 
-- 
CREATE OR REPLACE FUNCTION usercomments_upsert( 
cartodb_ids integer[], 
geojsons text[]) 
...
|| 'ST_SetSRID(ST_GeomFromGeoJSON(NULLIF('''|| geojsons[i] 
||''','''')),4326))'; 
END LOOP;  
...
GRANT EXECUTE ON FUNCTION usercomments_upsert(integer[],text[]) TO publicuser;